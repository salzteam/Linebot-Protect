...
#                    cl.sendMessage(to,"Profile Image Updated.")
            ...
            if cmd in [solo + " out", squad + " out"]:
            ...
            elif cmd in [solo + " tagall"]:
                group = cl.getChats(to);nama = [contact.mid for contact in cl.getChats([to]).chats[0].extra.groupExtra.memberMids];nama.remove(cl.getProfile().mid)
                cl.datamention(to,'Mentionall',nama)

            elif cmd in [solo + " mentions"]:
                members = []
                room = cl.getChats([to])
                members = [mem.mid for mem in cl.getChats([to]).chats[0].extra.groupExtra.memberMids]
                cl.tagall(to, members)
                ...
                if msg._from in owner or msg._from in creator:
                        ...
                        number = cmd.replace(separate[0] + ":","")
                        bot = status["bots"]
                        kntl = bot[int(number)-1]
                        del status["bots"][kntl]
                        cl.sendMention1(to,'Succes Delete @!        From Botlist',[kntl])
